...
def build_model(num_classes):
    try:
        ...
        print("\nĐang tạo SSD300 VGG16 với backbone pretrained...")
        model = ssd300_vgg16(
            weights=None,
        ...
        print("\nKhông dùng được backbone pretrained, chuyển sang train từ đầu.")
        print("Lý do:", e)
...
def plot_curves(metrics, out_dir):
    ...
    plt.title("Precision-Confidence Curve - SSD")
    ...
    plt.title("Recall-Confidence Curve - SSD")
    plt.xlabel("Confidence")
    plt.ylabel("Recall")
    plt.ylim(0, 1.05)
    plt.grid(True, alpha=0.3)
    ...
    plt.title("F1-Confidence Curve - SSD")
    plt.xlabel("Confidence")
    plt.ylabel("F1")
    plt.ylim(0, 1.05)
    plt.grid(True, alpha=0.3)
    ...
    plt.title("Precision-Recall Curve - SSD")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.xlim(0, 1)
    plt.ylim(0, 1.05)
...
def main():
    ...
    print("\nSố lớp SSD gồm background:", num_classes)
    print("Tên lớp:", names)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for images, targets in loop:
            ...
            if not torch.isfinite(losses):
                print("⚠️ Bỏ qua batch bị NaN/Inf loss")
                skipped_batches += 1
                continue
            ...
            bbox_loss = float(loss_dict.get("bbox_regression", torch.tensor(0.0)).item())
            cls_loss = float(loss_dict.get("classification", torch.tensor(0.0)).item())
        ...
        avg_bbox_loss = bbox_loss_sum / max(num_batches, 1)
        ...
        print(
            ...
            f"bbox={row['bbox_loss']:.4f} | "
            f"cls={row['cls_loss']:.4f} | "
    ...
    weight_path = os.path.join(OUT_DIR, "ssd_final.pth")
    ...
    plot_confusion_matrix(
        ...
        "Confusion Matrix Normalized - SSD",
        normalize=True
    )
